...
class AttendanceCalculator {
    ...
    public static function calculate($checkIn, $checkOut, $date, $empInfo, $settings) {
        ...
        if (!empty($empInfo['special_start_time'])) {
            $special_start = $empInfo['special_start_time'];
            $special_end = !empty($empInfo['special_end_time']) ? $empInfo['special_end_time'] : $default_end;

            $checkInTs = strtotime($checkIn);
            $checkOutTs = $checkOut ? strtotime($checkOut) : null;

            // Option A: Default Shift Penalties
            $expectedInDef = strtotime($date . ' ' . $default_start);
            $lateDef = max(0, (int)floor(($checkInTs - $expectedInDef) / 60) - $allowed_late);
            
            $earlyDef = 0;
            if ($checkOutTs) {
                $expectedOutDef = strtotime($date . ' ' . $default_end);
                if (strtotime($default_end) < strtotime($default_start)) $expectedOutDef = strtotime($date . ' ' . $default_end . ' +1 day');
                $earlyDef = max(0, (int)floor(($expectedOutDef - $checkOutTs) / 60));
            }

            // Option B: Special Shift Penalties
            $expectedInSpec = strtotime($date . ' ' . $special_start);
            $lateSpec = max(0, (int)floor(($checkInTs - $expectedInSpec) / 60) - $allowed_late);
            
            $earlySpec = 0;
            if ($checkOutTs) {
                $expectedOutSpec = strtotime($date . ' ' . $special_end);
                if (strtotime($special_end) < strtotime($special_start)) $expectedOutSpec = strtotime($date . ' ' . $special_end . ' +1 day');
                $earlySpec = max(0, (int)floor(($expectedOutSpec - $checkOutTs) / 60));
            }

            // Check if explicitly within default window
            $withinDefaultIn = ($lateDef <= 0);
            $withinDefaultOut = ($checkOutTs && $checkOutTs >= $expectedOutDef);

            // Decision: Use Default if it results in same or less penalty, or if explicitly within default window
            if (($withinDefaultIn && $withinDefaultOut) || ($lateDef + $earlyDef) <= ($lateSpec + $earlySpec)) {
                $work_start = $default_start;
                $work_end = $default_end;
            } else {
                $work_start = $special_start;
                $work_end = $special_end;
            }
        }
        ...
        $status = 'present';
        if ($isFlex) {
            if ($early_exit_minutes > 0) $status = 'early_exit';
        } else {
    ...
    public static function calculateDiscount($salary, $lateMinutes, $earlyExitMinutes, $workDayMinutes, $daysInMonth) {
        if ($salary <= 0 || $workDayMinutes <= 0 || $daysInMonth <= 0) return 0.0;
